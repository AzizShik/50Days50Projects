window.addEventListener('load', () => {
  ...
  const feedbackText = document.querySelector('.box__content-feedback'),
    userAvatar = document.querySelector('.box__content-img'),
    userName = document.querySelector('.box__content-name'),
    userJob = document.querySelector('.box__content-job'),
    progressBar = document.querySelector('.box__progress');
  ...
  function currentFeedback() {
    const currentFeedbackData = testimonials[idx];
    const {
      name,
      position,
      photo,
    ...
    userJob.textContent = position;

    if (idx < testimonials.length - 1) {
      setTimeout(nextTestimonials, nextTime);
    } else {
      idx = -1;
      setTimeout(nextTestimonials, nextTime);
    }
